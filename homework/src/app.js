  ...
  const reversedStr = newStr.replace(/,/g , ``);
  ...
  (loose or abstract equality) as JavaScript converts each to a string representation before comparison.
   "0" is a string with value 0 whereas [] represents empty array. Also there is a special rule about 
   comparison of []. When it is compared with a truthy, in this case "0" the result is false even though 
   [] is itself a truthy!  */
  ...
  console.log(`With %, first operand is divided by second operand and remainder is returned as a result.`);
